...
BEGIN
  IF OLD.salario != NEW.salario THEN
    ...
    VALUE (NEW.idFunc, NEW.nomeFunc, OLD.salario, NEW.salario, USER(), NOW());
  END IF;
END $$
DELIMITER ;
...
/*
Criar um trigger que controla os salários dos funcionários novos, 
considerando que eles não podem começar ganhando mais de R$2.000,00.
*/
...
BEGIN
   ...
   SELECT COUNT(idFunc)
   FROM Dependente
   WHERE idFunc = new.idFunc INTO vNumDeps;	
...
INSERT INTO Dependente (idDep, idFunc, nomeDep, dataNasc, sexo, parentesco) 
